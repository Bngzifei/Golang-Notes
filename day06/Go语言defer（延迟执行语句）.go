...
// Go语言中关键字defer允许我们推迟到函数返回之前(或任意位置执行return之后)一刻才执行某个语句或函数(为什么要在返回之后才执行这些语句?因为return语句同样可以包含一些操作,而不是单纯地返回某个值)
...
// 多个延迟执行语句的处理顺序:
// 党员多个defer行为被注册时,它们会以逆序执行(类似栈,即后进先出),下面的代码是将一系列的数值打印语句按顺序延迟处理,如下所示:
...
// defer 语句正好是在函数退出时执行的语句,所以使用defer能非常方便地处理资源释放问题
