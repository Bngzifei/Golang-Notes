...
// 这个接口需要实现Call方法,调用时会传入一个interface{}类型的变量,这种类型的变量表示任意类型的值
...
// 将定义的Struct类型实例化,并传入接口中进行调用,代码如下:
...
// 上面代码只是定义了函数类型,需要函数本身进行逻辑处理.FuncCall无须被实例化,只需要将函数转换为FuncCall类型即可,函数来源可以是命名函数,匿名函数或闭包,参见下面代码:
