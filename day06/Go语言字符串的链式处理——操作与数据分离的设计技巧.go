// 使用sql语言从数据库中获取数据时,可以对原始数据进行排序(sort by),分组(group by)和去重(distinct)操作.
// SQL将数据的操作与遍历过程作为两个部分进行隔离.这样操作和遍历过程就可以各自独立的进行设计,这就是常见的数据与操作分离的设计

// 对数据的操作进行多步骤的处理被称为链式处理.本例中使用多个字符串作为数据集合,
// 然后对每个字符串进行一系列的处理,用户可以通过系统函数或者自定义函数对链式处理中的每个环节进行自定义
