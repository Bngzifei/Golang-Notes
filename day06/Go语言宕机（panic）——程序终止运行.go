// Go语言宕机:
// Go语言的类型系统会在编译时捕获很多错误,但有些错误只能在运行时检查,如数组访问越界,空指针引用等,这些运行时的错误会引起宕机

// 宕机不是一件很好的事情,可能造成体验停止,服务中断.就像没有人希望在取钱时遇到ATM机蓝屏一样.但是,如果在损失发生时,程序没有因为宕机而停止,那么用户将会付出更大的代价,这种代价可以是金钱,时间甚至生命.因此,宕机有时是一种合理的止损方法.

// 一般而言,当宕机发生时,程序会中断运行,并立即执行在该goroutine(可以理解成线程)中被延迟的函数(defer机制).随后,程序崩溃并输出日志信息.日志信息包括panic value和函数调用的堆栈跟踪信息.panic value通常是某种错误信息

// 对于每个goroutine,日志信息中会有与之对应的,发生panic时的函数调用堆栈跟踪信息.通常,我们不需要再次运行程序去定位问题,日志信息已经提供了足够的诊断依据.因此,在我们填写问题报告时,一般会将宕机和日志信息一并记录.

// 虽然Go语言的panic机制类似于其他语言的异常,但,panic的适用场景有一些不同.由于panic会引起程序的崩溃,因此panic一般用于严重错误,如程序内部的逻辑不一致.勤奋的程序员认为任何崩溃都表明代码中存在漏洞,所以对于大部分漏洞,我们应该使用Go提供的错误机制,而不是panic,尽量避免程序的崩溃.

// 手动触发宕机
// Go语言可以在程序中手动触发宕机,让程序崩溃这样开发者可以及时地发现错误,同时减少可能的损失.

// Go语言程序在宕机时,会将堆栈和goroutine信息输出到控制台,所以宕机也可以方便地知晓发生错误的位置.如果在编译时加入的调试信息甚至连崩溃现场的变量值,运行状态都可以获取,那么如何触发宕机?例如下面的代码:
...
// panic的参数可以是任意类型,后文将提到的revover参数会接收从panic中发出的内容
