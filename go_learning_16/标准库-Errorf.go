...
/*
fmt.Scanf不同于fmt.Scan简单的以空格作为输入数据的分隔符,fmt.Scanf为输入数据指定了
具体的输入内容格式,只有按照格式输入数据才会被扫描并存入对应变量.

例如,我们还是按照上个示例中以空格分隔的方式输入,fmt.Scanf就不能正确扫描到输入的数据.

fmt.Scanln:函数签名如下:
func Scanln(a ...interface{}) (n int,err error)
	Scanln类似Scan,它在遇到换行时才停止扫描.最后同一个数据后面必须有换行或者到达结束位置.
本函数返回成功扫描的数据个数和遇到的任何错误.
...
有时候我们想完整获取输入的内容,而输入的内容可能包含空格,这种情况下可以使用bufio包
来实现.
*/
